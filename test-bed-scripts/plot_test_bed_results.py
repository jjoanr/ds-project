...
for packet_loss_results in DATA_PATH.iterdir():
    ...
    df = pd.read_csv(packet_loss_results / f"naive_summary_{packet_loss}_loss.csv")
...
# Plot
packet_loss_values = list(results.keys())
...
plt.legend()
plt.tight_layout()
plt.savefig("naive_mre_testbed.png")
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig("naive_reliability_testbed.png")
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig("naive_traffic_testbed.png")
plt.close()
